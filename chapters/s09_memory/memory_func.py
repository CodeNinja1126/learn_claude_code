...
def _completion_text(prompt: str, max_tokens: int) -> str:
    """Run a small OpenAI-compatible chat completion and return text content."""
    config = load_config()
...
def select_relevant_memories(messages: list, max_items: int = 5) -> list[str]:
    ...
    try:
        text = _completion_text(prompt, max_tokens=200).strip()
...
def extract_memories(messages: list):
    ...
    try:
        text = _completion_text(prompt, max_tokens=800).strip()
        # Extract JSON array from response
        match = re.search(r"\[.*\]", text, re.DOTALL)
        if not match:
            return
...
def consolidate_memories():
    ...
    try:
        text = _completion_text(prompt, max_tokens=3000).strip()
        match = re.search(r"\[.*\]", text, re.DOTALL)
        if not match:
            return
        items = json.loads(match.group())
