...
HOOKS: dict[str, list[Callable[..., str | None]]] = {
    "UserPromptSubmit": [],
    "PreToolUse": [],
    "PostToolUse": [],
    "Stop": [],
...
def permission_hook(name: str, args: dict[str, Any] | None = None) -> str | None:
    args = args or {}
    if name == "bash":
...
def log_hook(name: str, args: dict[str, Any]) -> str | None:
    print(f"[HOOK] {name} 사용되었음.")
    return None


def large_output_hook(name: str, output: Any) -> str | None:
...
def summary_hook(messages: list[dict[str, Any]]) -> str | None:
    ...
    tool_count = sum(1 if m["role"] == "tool" else 0 for m in messages)
